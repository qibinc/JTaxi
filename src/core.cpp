...
const char Data_File[]="../data/GP_Tree.data";
const char Car_File[]="../data/taxi.data";
...
void load_vector(FILE *f, vector<int> &v)
{
    v.clear();
    int n,i,j;
    fscanf(f, "%d",&n);
...
void load_vector_vector(FILE *f, vector<vector<int> > &v)
{
    v.clear();
    int n,i,j;
    fscanf(f, "%d",&n);
...
void load_vector_pair(FILE *f, vector<pair<int,int> > &v)
{
    v.clear();
    int n,i,j,k;
    fscanf(f, "%d",&n);
...
void load_map_int_pair(FILE *f, map<int,pair<int,int> > &h)
{
    int n,i,j,k,l;
    fscanf(f, "%d",&n);
    for(i=0;i<n;i++)
...
void load_map_int_int(FILE *f, map<int,int> &h)
{
    int n,i,j,k;
    fscanf(f, "%d",&n);
    for(i=0;i<n;i++)
...
struct Graph//无向图结构 
{
    ...
    void load(FILE *f)//读取结构信息(stdout输出)
    {
        fscanf(f, "%d%d%d",&n,&m,&tot);
        load_vector(f, id);
        load_vector(f, head);
        load_vector(f, list);
        load_vector(f, next);
...
struct G_Tree
{
    ...
    struct Node
    {
        ...
        void load(FILE *f)
        {
            fscanf(f, "%d%d%d%d%d%d%d",&n,&father,&part,&deep,&catch_id,&catch_bound,&min_border_dist);
            if(son!=NULL)delete[] son;
            son=new int[part];
            for(int i=0;i<part;i++)fscanf(f, "%d",&son[i]);
            load_vector(f, color);
            dist.load(f);
            order.load(f);
            load_map_int_pair(f, borders);
            load_vector(f, border_in_father);
            load_vector(f, border_in_son);
            load_vector(f, border_id);
            load_vector(f, border_id_innode);
            load_vector(f, path_record);
            load_vector(f, catch_dist);
            load_vector_pair(f, min_car_dist);
        }
        void init(int n)
        {
            part=n;
    ...
    {
        G.load(f);
        fscanf(f, "%d%d%d",&root,&node_tot,&node_size);
        load_vector(f, id_in_node);
        load_vector_vector(f, car_in_node);
        load_vector(f, car_offset);
...
void optimalOrder_(vector<int> & passenger, int step, vector<bool> & v, int & ans, int last, int sum)
{
    if (sum >= ans) {
        return ;
    }
    ...
    if (step == n) {
        ...
        for (int i = 0; i < n; ++ i) {
            if (!v[i]) {
                ...
                optimalOrder_(passenger, step + 1, v, ans, x, sum + delta);
                v[i] = false;
            }
        }
    }
...
int optimalOrder(vector<int> passengers_pos, int taxi_pos) {
    int ans = 1e9;
    vector<bool> v(5, false);
    optimalOrder_(passengers_pos, 0, v, ans, taxi_pos, 0);
    return ans;
}

void optimalOrderRoute_(vector<int> & passenger, int step, vector<bool> & v, 
    ...
    if (step == n) {
        ...
        for (int i = 0; i < n; ++ i) {
            if (!v[i]) {
                ...
                route[step] = x;
                int delta = tree.search_catch(last, x);				
                optimalOrderRoute_(passenger, step + 1, v, ans, x, sum + delta, route, ans_route);
                v[i] = false;
            }
        }
    }
...
vector<int> optimalOrderRoute(vector<int> passengers_pos, int taxi_pos) {
    int ans = 1e9;
    vector<bool> v(5, false);
    vector<int> route(passengers_pos.size()), res_route(passengers_pos.size());
    optimalOrderRoute_(passengers_pos, 0, v, ans, taxi_pos, 0, route, res_route);
    return res_route;
}
...
json searchTaxi(int S, int T, int K)
{
    ...
    while (!Q.empty()) {
        ...
        for (int j = 0; j < t; ++ j) {
            ...
            int D3 = optimalOrder(now_pass, S);
...
vector<int> wholePath(vector<int> nodes)
{
    vector<int> tmp, res;
    for (int i = 0; i + 1 < nodes.size(); ++ i) {
        tmp.clear();
        tree.find_path(nodes[i], nodes[i + 1], tmp);
        for (int j = 0; j < tmp.size(); ++ j) {
            res.push_back(tmp[j]);
        }
    }
    if (nodes.size() > 0) {
        res.push_back(nodes[nodes.size() - 1]);
    }
    return res;
}
...
int main()
{
    ...
    while (getline(cin, line)) {
        ...
        if (input["name"] == "searchTaxi") {
            cout << json(searchTaxi(input["s"], input["t"], input["k"])) << endl;
        }
        else if (input["name"] == "optimalOrder") {
            cout << json(optimalOrder(input["passengers"], input["taxi"])) << endl;
        }
        else if (input["name"] == "optimalOrderRoute") {
            cout << json(optimalOrderRoute(input["passengers"], input["taxi"])) << endl;
        }
        else if (input["name"] == "wholePath") {
            cout << json(wholePath(input["nodes"])) << endl;
        }
    }
